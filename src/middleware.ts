...
export async function middleware(request: NextRequest) {
  ...
  // Usar apenas variáveis NEXT_PUBLIC_ para consistência
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

  // Se as variáveis não estiverem configuradas, permitir acesso mas com log
  if (!supabaseUrl || !supabaseAnonKey) {
    console.warn(
      "Supabase environment variables not configured. Allowing access without authentication."
    );
